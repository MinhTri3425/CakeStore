...
import com.cakestore.cakestore.repository.admin.AdminAddressRepository;
...
public class UserServiceImpl implements UserService {
    ...
    private final AdminAddressRepository addressRepository;
    private final AdminBranchRepository branchRepository;

    public UserServiceImpl(AdminUserRepository userRepository, PasswordEncoder passwordEncoder,
            AdminAddressRepository addressRepository, AdminBranchRepository branchRepository) {
    ...
    @Override
    @Transactional
    public User save(User user, String rawPassword, String line1, String city, String district, String ward,
            Long branchId) {
        User savedUser;
        ...
        if (line1 != null && !line1.isBlank() && city != null && !city.isBlank()) {
            Address defaultAddress = addressRepository.findByUserIdAndIsDefaultTrue(savedUser.getId())
                    .orElseGet(() -> {
                        Address newAddr = new Address(savedUser, savedUser.getFullName(), savedUser.getPhone(), line1,
                                city);
