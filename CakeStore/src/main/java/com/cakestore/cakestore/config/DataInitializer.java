    ...
    public CommandLineRunner initDatabase(UserRepository userRepository, PasswordEncoder passwordEncoder) {
        return args -> {
            ...
            if (userRepository.findByEmail("admin@cakestore.com").isEmpty()) {
                User admin = new User(
                    "admin@cakestore.com", 
                    passwordEncoder.encode("adminpassword"), // Mật khẩu gốc: adminpassword
                    "System Administrator"
                );
                admin.setRole("admin"); // Đặt role là admin
                userRepository.save(admin);
                System.out.println(">> Created Admin account: admin@cakestore.com / adminpassword");
            }
            ...
            if (userRepository.findByEmail("staff@cakestore.com").isEmpty()) {
                User staff = new User(
                    "staff@cakestore.com", 
                    passwordEncoder.encode("staffpassword"), // Mật khẩu gốc: staffpassword
                    "Cake Store Staff"
                );
                staff.setRole("staff"); // Đặt role là staff
                userRepository.save(staff);
                System.out.println(">> Created Staff account: staff@cakestore.com / staffpassword");
            }
            ...
            if (userRepository.findByEmail("user@cakestore.com").isEmpty()) {
                User user = new User(
                    "user@cakestore.com", 
                    passwordEncoder.encode("userpassword"), // Mật khẩu gốc: userpassword
                    "Regular Customer"
                );
                user.setRole("customer"); // Đặt role là customer
                userRepository.save(user);
                System.out.println(">> Created Customer account: user@cakestore.com / userpassword");
            }
