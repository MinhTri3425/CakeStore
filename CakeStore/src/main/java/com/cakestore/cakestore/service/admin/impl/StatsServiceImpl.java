...
import com.cakestore.cakestore.repository.admin.OrderItemRepository;
import com.cakestore.cakestore.repository.admin.OrderRepository;
...
public class StatsServiceImpl implements StatsService {
    ...
    public StatsServiceImpl(OrderRepository orderRepository, ProductRepository productRepository, UserRepository userRepository, OrderItemRepository orderItemRepository) { // Cập nhật constructor
    ...
    public Map<String, Object> getDashboardStats() {
        ...
        List<OrderStatus> revenueStatuses = List.of(OrderStatus.DELIVERED, OrderStatus.SHIPPING, OrderStatus.CONFIRMED);
    ...
    public Map<String, Object> getDetailedStats() {
        ...
        List<OrderStatus> revenueStatuses = List.of(OrderStatus.DELIVERED, OrderStatus.SHIPPING, OrderStatus.CONFIRMED);
        List<Map<String, Object>> dailyRevenueRaw = orderRepository.findDailyRevenueSince(revenueStatuses, sevenDaysAgo);
        Map<LocalDate, BigDecimal> revenueByDate = dailyRevenueRaw.stream()
                .collect(Collectors.toMap(
                        row -> LocalDate.parse((String) row.get("orderDate"), DateTimeFormatter.ofPattern("yyyy/MM/dd")),
        ...
        long totalOrders = orderStatusCountsRaw.stream().mapToLong(row -> (Long) row.get("count")).sum(); // Tính tổng số đơn
        ...
        for (OrderStatus status : orderedStatuses) {
            ...
            if (count > 0 || totalOrders > 0) { // Chỉ thêm nếu có hoặc để tính % 0
                ...
                orderStatusData.add(Math.round(percentage * 10.0) / 10.0); // Làm tròn 1 chữ số thập phân
            }
        }
